   ...
   private:
      ...
      void initPool(size_t bufSize)
      {
         for (int i = 0 ; i < SIZE ; ++i) {
            try {
#if USE_CXX11
               std::unique_ptr<TYPE> buf(new TYPE[bufSize + sizeT<PoolIt, TYPE>::value]);
#else
               std::auto_ptr<TYPE> buf(new TYPE[bufSize + sizeT<PoolIt, TYPE>::value]);
#endif
               _buf.push_back(buf.get());
